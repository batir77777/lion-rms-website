...
describe("The 404 stays out of the sitemap and out of search", () => {
  test("no sitemap entry points at a 404 route", async () => {
    ...
    assert.equal(entries.length, 83, `the sitemap should still list 83 URLs, found ${entries.length}`);
  ...
  test("the index still holds exactly 54 pages", () => {
    ...
    // filtering it. 52 through PR6; +1 for
    // guides/fire-compartmentation-survey-explained; +1 for PR 7's
    ...
    assert.equal(fragments.length, 54, `expected 54 indexed pages, found ${fragments.length}`);
  });
});
